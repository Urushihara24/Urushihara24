<div align="center">

# Привет, я Всеволод! 👋

### Junior QA Engineer | Python Developer

*Я — начинающий QA-инженер с фокусом на ручном тестировании, автоматизации и разработке инструментов на Python. Мне нравится находить нетривиальные проблемы в продуктах и создавать решения, которые делают работу команды эффективнее.*

<div align="center">

[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Fira+Code&pause=1000&color=F75C7E&center=true&vCenter=true&width=435&lines=QA+Engineer;Python+Developer;Automation+Enthusiast)](https://git.io/typing-svg)

</div>

---

</div>

## 💻 Ключевые навыки

<details open>
<summary><b>🧪 Тестирование</b></summary>
<br>

- **Методологии:** Ручное функциональное, API-тестирование, UI/UX, Регрессионное
- **Инструменты:** Postman, Charles Proxy, DevTools, ADB
- **Документация:** Тест-кейсы, чек-листы, баг-репорты

</details>

<details>
<summary><b>💻 Разработка</b></summary>
<br>

- **Языки:** Python, SQL
- **Python библиотеки:** `requests`, `pandas`, `openpyxl`, `gspread`, `aiogram`, `beautifulsoup4`
- **Базы данных:** PostgreSQL, MySQL
- **Инструменты:** Git, GitHub, Jira

</details>

<details>
<summary><b>🔧 Дополнительно</b></summary>
<br>

- **API:** Telegram Bot API, Google Sheets API
- **Автоматизация:** Парсинг данных, интеграция с внешними сервисами
- **Форматы:** JSON, XML, CSV, Excel

</details>

---

## 🚀 Мои проекты

> *Здесь я делюсь своими самыми интересными проектами. Это не просто учебные задания, а реальные инструменты, которые я создавал по собственной инициативе для решения конкретных проблем.*

<table>
<tr>
<td width="50%">

### 🤖 [Exportum Bot](https://github.com/Urushihara24/Exportum_bot)
**Telegram-бот для торговли зерновыми**

Комплексное решение с интеграцией Google Sheets, управлением сделками, логистикой и парсингом цен с сайтов.

**Технологии:** `Python` `aiogram` `Google Sheets API` `BeautifulSoup` `FSM`

</td>
<td width="50%">

### 📊 [Скрипт для склада](https://github.com/Urushihara24/warehouse-capacity-script)
**Автоматизация складских мощностей**

Сократил время расчетов с часов до минут благодаря автоматизированному анализу и отчетности.

**Технологии:** `Python` `SQL` `pandas` `Google Sheets API`

</td>
</tr>
<tr>
<td width="50%">

### 📱 [Приложение WBHelp](https://github.com/Urushihara24/employee-feedback-app)
**Сервис обратной связи**

Многофункциональный сервис для операционных задач команды из 50+ человек.

**Технологии:** `Python` `HTML/CSS` `Android UI` `Telegram API`

</td>
<td width="50%">

### 🧪 [QA Portfolio](https://github.com/Urushihara24/qa-portfolio)
**Портфолио тестировщика**

Чек-листы, тест-кейсы и баг-репорты для приложений "Яндекс Метро" и "Яндекс.Маршруты".

**Навыки:** `Testing` `Documentation` `API Testing` `UI/UX`

</td>
</tr>
</table>

---

## 📊 GitHub Analytics

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=Urushihara24&show_icons=true&theme=radical&include_all_commits=true&count_private=true"/>
<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Urushihara24&layout=compact&theme=radical"/>

</div>

<div align="center">
  
![](https://github-readme-streak-stats.herokuapp.com/?user=Urushihara24&theme=radical&hide_border=false)

</div>

---

## 🛠️ Технологический стек

<div align="center">

![Python](https://img.shields.io/badge/-Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/-SQL-4479A1?style=for-the-badge&logo=postgresql&logoColor=white)
![Git](https://img.shields.io/badge/-Git-F05032?style=for-the-badge&logo=git&logoColor=white)

![Postman](https://img.shields.io/badge/-Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white)
![Jira](https://img.shields.io/badge/-Jira-0052CC?style=for-the-badge&logo=jira&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-336791?style=for-the-badge&logo=postgresql&logoColor=white)

</div>

---

## 📈 Опыт работы

### 🏢 **Wildberries** | *2023-2024*
**Старший специалист складского комплекса**

- 🔧 Разработал Python-скрипт для автоматизации анализа складских мощностей
- 📱 Создал приложение для обратной связи сотрудников (50+ пользователей)
- ⚡ Оптимизировал рабочие процессы через автоматизацию рутинных задач

---

## 🎓 Образование

- 📚 **Яндекс.Практикум** — *Курс "Инженер по тестированию"* (2024)
- 💼 **Self-taught** — *Python разработка и автоматизация* (2023-2025)

---

## 📫 Связаться со мной

<div align="center">

[![Email](https://img.shields.io/badge/-Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:i@urushihara24.ru)
[![Telegram](https://img.shields.io/badge/-Telegram-2CA5E0?style=for-the-badge&logo=telegram&logoColor=white)](https://t.me/urushihara24)
[![GitHub](https://img.shields.io/badge/-GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Urushihara24)

**📧 Email:** i@urushihara24.ru  
**💬 Telegram:** @urushihara24  
**📍 Локация:** Казань, Россия

</div>

---

## 🌟 Интересы & Цели

<table>
<tr>
<td width="50%">

### 🌟 **Интересы**
- 🤖 Создание Telegram-ботов
- 📊 Автоматизация работы с данными  
- 🧪 Тестирование и обеспечение качества
- 🚀 Изучение новых технологий

</td>
<td width="50%">

### 💡 **Цели на 2025**
- [ ] Освоить автоматизацию тестирования (Selenium, pytest)
- [ ] Углубить знания в API тестировании  
- [ ] Получить сертификацию ISTQB
- [ ] Создать 5+ open-source проектов
- [ ] Участвовать в open-source сообществе

</td>
</tr>
</table>

---

<div align="center">

![](https://komarev.com/ghpvc/?username=Urushihara24&color=blueviolet&style=for-the-badge)

### ⭐️ *From [Urushihara24](https://github.com/Urushihara24) with* ❤️

</div>
